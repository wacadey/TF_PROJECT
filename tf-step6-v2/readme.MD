...
2. [v]`02_variables.tf` — 외부 입력값 -> tfvars 로 세팅값으로 변경 가능함
...
~/tf-step6> terraform -chdir="./infra"  apply
---
aws 콘솔 > vpc > 리소스맵 > 네트워크 구성 확인

~/tf-step6> terraform -chdir="./infra"  destroy
```
6. [v]`06_iam.tf` — EKS Cluster Role과 Auto Mode Node Role
```
- 삭제시 해당 role이 삭제 않되면 => 자동화 구성시 오류 발생함(중복 역활 생성)
```
...
10. [v]`10_ecr.tf` — WEB/WAS 이미지 저장소와 이미지 정리 정책 - ci/cd 관련
...
- 생성후 체크
    - 사이트 접속
        ...
        - 않보이면
            ...
            public-alb   alb     *       k8s-deai25-publical-688006d84a-567167101.us-east-1.elb.amazonaws.com   80      16m
            ```
        - http://k8s-deai25-publical-688006d84a-567167101.us-east-1.elb.amazonaws.com

    - EKS에 접속(본인 리전)
        - 클러스터 확인 > 상세 정보 > 각종 정보 확인 가능
            - 대시보드상 각종 메뉴에 리소스 확인
        - ec2 확인
            - node(ec2)는 ec2 대시보드에서 보이지 x
        - rds 확인
        - ecr 확인
        - vpc 확인
        - Secrets Manager 확인
